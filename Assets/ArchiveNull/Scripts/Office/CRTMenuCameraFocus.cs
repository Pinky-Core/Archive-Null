...
namespace ArchiveNull.UI
{
    public sealed class CRTMenuCameraFocus : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                _vrHeadsetStarter = FindAnyObjectByType<VRHeadsetArchiveStarter>();
            }
        ...
        private void HandleInput()
        {
            ...
            if (!_isFocused && _currentPose == CameraPose.Far && Mouse.current != null && Mouse.current.rightButton.wasPressedThisFrame)
            {
                MoveToStandPose();
                return;
            }

            if (!_isFocused && _currentPose == CameraPose.Stand && WasEscapePressedThisFrame())
            {
                FocusComputer();
                return;
            }
