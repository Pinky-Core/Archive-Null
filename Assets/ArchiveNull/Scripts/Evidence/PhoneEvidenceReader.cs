...
{
    /// <summary>
    /// Controls the collectible phone state machine, navigation and runtime-generated UI.
    /// </summary>
    ...
    public sealed class PhoneEvidenceReader : MonoBehaviour
    {
        ...
        private readonly HashSet<PhoneScreen> narratedScreens = new();
        ...
        public void Collect()
        {
            ...
            EvidenceCameraController controller = FindAnyObjectByType<EvidenceCameraController>();
            if (controller == null)
            {
                return;
            }
        ...
        public void SetEquippedState(bool value)
        {
            ...
            ShowScreenNarration(currentScreen);
        ...
        private void Unlock()
        {
            ...
            ShowScreenNarration(PhoneScreen.Home);
            selectedItem = 0;
            RefreshScreen();
        }
        ...
        private void OpenChat(int index)
        {
            ...
            ShowScreenNarration(PhoneScreen.Chat);
            selectedItem = 0;
            RefreshScreen();
        }
        ...
        private void CreateChatBubble(RectTransform parent, string raw)
        {
            ...
            bubble.rectTransform.anchorMin = new Vector2(outgoing ? 0.18f : 0.03f, 0f);
            bubble.rectTransform.anchorMax = new Vector2(outgoing ? 0.97f : 0.82f, 1f);
            bubble.rectTransform.offsetMin = Vector2.zero;
            bubble.rectTransform.offsetMax = Vector2.zero;
            ...
            const float availableTextWidth = 276f;
        ...
        private void ShowScreenNarration(PhoneScreen screen)
        {
            if (!narratedScreens.Add(screen) || EvidenceGuidanceController.ExistingInstance == null) return;
            string line = screen switch
            {
                PhoneScreen.Lock => GameLocalization.Text("La pantalla está bloqueada. Necesito acceder sin alterar el contenido.", "The screen is locked. I need to gain access without altering its contents."),
                PhoneScreen.Home => GameLocalization.Text("Mensajes y llamadas son las aplicaciones relevantes. La galería no está disponible en esta reconstrucción.", "Messages and calls are the relevant applications. The gallery is unavailable in this reconstruction."),
                PhoneScreen.ChatList => GameLocalization.Text("Los contactos pueden construir una línea temporal, pero una conversación aislada no demuestra culpabilidad.", "Contacts can establish a timeline, but an isolated conversation does not prove guilt."),
                PhoneScreen.Chat => GameLocalization.Text("El tono y la longitud importan. Tengo que comparar estos mensajes con la despedida final.", "Tone and length matter. I need to compare these messages with the final farewell."),
                PhoneScreen.Calls => GameLocalization.Text("El registro de llamadas puede confirmar horarios y contradicciones en los testimonios.", "The call log may confirm times and contradictions in witness statements."),
                _ => string.Empty
            };
